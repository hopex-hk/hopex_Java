...
public class ConnectionFactory {
  ...
  private static final OkHttpClient client = new OkHttpClient.Builder().followSslRedirects(false).followRedirects(false)
  ...
  public static String execute(Request request) {
    ...
    try {
      ...
      if (response.code() != 200) {
        throw new HopexException(HopexException.EXEC_ERROR,
      ...
      if (response != null && response.body() != null) {
        ...
        throw new HopexException(HopexException.ENV_ERROR, "[Execute] Cannot get the response from server");
      }
      log.debug("[Response]{}", str);
      return str;
    } catch (IOException e) {
      e.printStackTrace();
      throw new HopexException(HopexException.RUNTIME_ERROR, "[Execute] Cannot get the response from server");
    }
