...
public class InputChecker {
  ...
  public <T> InputChecker shouldNotNull(T value, String name) {
    if (value == null) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] " + name + " should not be null");
    }
    return checkerInst;
  }
  ...
  public <T> InputChecker shouldNull(T value, String name) {
    if (value != null) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] " + name + " should be null");
    }
    return checkerInst;
  }
  ...
  public InputChecker checkSymbol(String symbol) {
    if (symbol == null || "".equals(symbol)) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] Symbol is mandatory");
    }
    ...
    if (isSpecialChar(symbol)) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] " + symbol + " is invalid symbol");
    }
    return checkerInst;
  }
  ...
  public InputChecker checkCurrency(String currency) {
    if (currency == null || "".equals(currency)) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] Currency is mandatory");
    }
    if (isSpecialChar(currency)) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] " + currency + " is invalid currency");
    }
    return checkerInst;
  }
  ...
  public InputChecker checkRange(int size, int min, int max, String name) {
    if (!(min <= size && size <= max)) {
      throw new HopexException(HopexException.INPUT_ERROR,
          "[Input] " + name + " is out of bound. " + size + " is not in [" + min + "," + max + "]");
    }
    return checkerInst;
  }

  public InputChecker checkRange(BigDecimal size, BigDecimal min, BigDecimal max, String name) {
    if (!(min.compareTo(size) < 1 && size.compareTo(max) < 1)) {
      throw new HopexException(HopexException.INPUT_ERROR,
          "[Input] " + name + " is out of bound. " + size + " is not in [" + min + "," + max + "]");
    }
  ...
  public InputChecker greaterOrEqual(Integer value, int base, String name) {
    if (value != null && value < base) {
      throw new HopexException(HopexException.INPUT_ERROR, "[Input] " + name + " should be greater than " + base);
    }
    return checkerInst;
  }
